...
<!-- Synced from docs/configuration.md in the CertPilot repository by
...
Start from [`config.example.yaml`](https://github.com/certpilot/certpilot/blob/main/config.example.yaml).
...
| `CERTPILOT_KEK_RETIRED` | no | Comma-separated previous KEKs, still able to decrypt. See [KEK rotation](/operations#rotating-the-kek) |
...
its own. See [gateways/vault.md](/gateways/vault).
...
with a revoked credential or an unreachable core. See [agent.md](/agent).
...
load ([`pkg/config/config.go`](https://github.com/certpilot/certpilot/blob/main/pkg/config/config.go) `Validate`); the last
...
configured. See [operations.md](/operations) and [security.md](/security).
