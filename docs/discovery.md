...
<!-- Synced from docs/discovery.md in certpilot/certpilot at 9e9a56600009,
...
Plus a fourth, from the inside: the [agent's file inventory](/agent#inventory).
...
[ACM and Key Vault deployers](/deployment) so one set of credentials serves
...
store-divergence class will come from. Recorded in [database.md](/database).
